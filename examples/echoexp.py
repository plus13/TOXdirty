...
class EchoBot(Tox):
    ...
    def on_friend_message(self, friendId, type, message):
        ...
        print(message)
        if message == "?send":
            self.file_send(friendId, 65536, getsize("test.txt"), "0", "test.txt")

    def on_file_recv(self, fid, filenumber, kind, size, filename):
        print(fid, filenumber, kind, size, filename)
        print(type(fid), type(filenumber), type(kind), type(size), type(filename))
        self.file_control(fid, filenumber, Tox.FILE_CONTROL_RESUME)

    def on_file_recv_chunk(self, fid, file_number, position, data):
        '''this methods gets called when data is send. It saves the data.'''
        print("Hello2")
        print(fid, file_number, position, data)
        if type(data) is type(b'byte'):
    ...
    def on_file_chunk_request(self, friend_number, file_number, position, length):
        print("File sending: friend_number, file_number, position, length:", friend_number, file_number, position, length)
        if self.data is "":
            read_file = open("test.txt", "rb")  # open for [r]eading as [b]inary
            self.data = read_file.read()
            read_file.close()
        # Only chunks can be send, so split it in chunks.
        data_chunk = self.data[position:(position + length)]
        try:
            if length == 0:
                self.data = ""
                return
            self.file_send_chunk(friend_number, file_number, position, data_chunk)
